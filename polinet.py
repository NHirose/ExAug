...
class PoliNet_exaug(nn.Module):
    def __init__(self, in_chs, step_size, lower_bounds, upper_bounds):
        super(PoliNet_exaug, self).__init__()
    ...
    def forward(self, img, size, fsize, px, pz, ry):
        x1 = self.model_1(img)
        #print(x1.size(), size.size())
        #print(size.repeat(1,1,8,8))
        ...
        x3 = torch.cat((x1, size.repeat(1,1,1,1), pose_pred.unsqueeze(2).unsqueeze(3)), axis=1)
        x3f = torch.flatten(x3, start_dim=1)
        p = self.model_3(x3f)

        x3_x = torch.cat((x1, fsize.repeat(1,1,1,1), pose_pred.unsqueeze(2).unsqueeze(3)), axis=1)
        x3f_x = torch.flatten(x3_x, start_dim=1)
        p_x = self.model_3(x3f_x)

        return self.out_scale * y + self.out_offset, p, p_x
